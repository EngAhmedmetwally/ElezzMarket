...
import { DailyProductChart } from './daily-product-chart';
...
export default function ProductOrdersPage() {
  ...
  const productOrders = React.useMemo((): ProductOrder[] => {
    ...
    const filtered = allOrdersInRange.filter(order => {
      const itemsArray: OrderItem[] = order.items ? (Array.isArray(order.items) ? order.items : Object.values(order.items)) : [];
  ...
  if (isLoading) {
    return (
        <div>
          <PageHeader title={`${language === 'ar' ? 'تحميل تقرير المنتج...' : 'Loading Product Report...'}`} />
  ...
  return (
    <div className="space-y-8">
      <PageHeader title={`${language === 'ar' ? 'تقرير المنتج:' : 'Report for:'} ${productName}`} />
      ...
      <DailyProductChart data={productOrders} productName={productName} />
