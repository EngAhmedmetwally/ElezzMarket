...
const commissionableStatuses: OrderStatus[] = ["تم التسجيل", "قيد التجهيز", "تم الشحن", "مكتمل", "ملغي", "معلق"];

const commissionFormSchema = z.object({
  "تم التسجيل": z.coerce.number(),
  "قيد التجهيز": z.coerce.number(),
  "تم الشحن": z.coerce.number(),
  "مكتمل": z.coerce.number(),
  "ملغي": z.coerce.number(),
  "معلق": z.coerce.number(),
});
...
export default function CommissionsPage() {
  ...
  async function onSubmit(data: CommissionFormValues) {
    ...
    try {
        ...
        commissionableStatuses.forEach(status => {
            rulesToSave[status] = { amount: data[status as keyof CommissionFormValues] || 0 };
        });
    ...
    <div className="space-y-8">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {language === 'ar' ? 'قم بتعيين مبالغ العمولات لكل حالة من حالات الطلب. يمكنك استخدام قيم سالبة للخصم.' : 'Set commission amounts for each order status. You can use negative values for deductions.'}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Form {...form}>
                ...
                {commissionableStatuses.map((status) => {
                  ...
                  return (
                    <TableRow key={status}>
                      ...
                      <TableCell className="text-end">{formatCurrency(rule?.amount || 0, language)}</TableCell>
                    </TableRow>
                  );
                })}
              </TableBody>
